...
async function renderEvents() {
  ...
  try {
    ...
    const now = Date.now();
    events = events
      .filter(e => new Date(e.date).getTime() >= now - 12 * 3600 * 1000)
      ...
      return `
        <div class="event-item">
          <div class="event-date">
            <div class="event-date__day">${d.getDate()}</div>
            <div class="event-date__ym">${d.getFullYear()}.${d.getMonth() + 1}</div>
          </div>
          <div class="event-body">
            <p class="event-title">${CoAX.escapeHTML(e.title)}</p>
            <p class="event-meta">
              <span class="tag ${tagClass}">${CoAX.escapeHTML(e.type)}</span>
              <span>${CoAX.formatDate(e.date, true)}</span>
